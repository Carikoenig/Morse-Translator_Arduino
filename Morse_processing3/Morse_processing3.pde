...
void draw() {
  ...
  if (port.available() > 0) { // If data is available,
    ...
    if (input != null) {
      /*
      // TODO: work on this. Read out the info if 1 and long or short press
      if (input == "1") {
        //count a counter upwards
      } else {
        //counter to 0
      }
      */
      println("Input: " + input);
      /*
      // debugging and trying to find the String to int eror
      buttonState = int(input);
      println("Input: " + input);
...
void serialEvent(Serial myPort){
 buttonState = float (myPort.readStringUntil('\n'));
 println(buttonState);
 }
 */
